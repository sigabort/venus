...
//     vs.setInstitute(institute);
  ...
  public void testFindUserWithoutUsername() throws Exception {
    ...
    Assert.fail("findUserByUsername didn't throw exception when username is not passed");
  }
  ...
  public void testFindNonActiveUser() throws Exception {
   ...
   Assert.assertEquals("User name", name, user.getUsername());
   Assert.assertEquals("User Institute Id", vs.getInstituteId(), (Integer)user.getInstituteId());
   Assert.assertEquals("User status", (int)Status.Unverified.ordinal(), (int)user.getStatus());
   Assert.assertNotNull("user created date", user.getCreated());
   Assert.assertNotNull("user last modified date", user.getLastModified());   
   /* test the optional parameters */
   testUserOptionalParams(user, params);
   ...
   Assert.assertEquals("The user found is not same as original user", user, nuUser);
   /* check optional parameters too */
   testUserOptionalParams(nuUser, params);
  ...
  public void testGetUsers() throws Exception {
   ...
   Assert.assertEquals("user institute Id", vs.getInstituteId(), user1.getInstituteId());
   ...
   Assert.assertEquals("user institute Id", vs.getInstituteId(), user2.getInstituteId());
  ...
  public void testGetUsersSortOnId() throws Exception {
   ...
   Assert.assertNotNull("didn't get any users", list);
   Assert.assertEquals("list should only contain 2 users", (new Integer(2)).intValue(), (int)list.size());
   User u1 = (User) list.get(0);
   Assert.assertEquals("first one should be equal to the one created first", user1, u1);
   User u2 = (User) list.get(1);
   Assert.assertEquals("second one should be equal to the one created later", user2, u2);
   ...
   Assert.assertEquals("list should only contain 2 users", (new Integer(2)).intValue(), (int)list.size());
   u1 = (User) list.get(0);
   Assert.assertEquals("first one should be equal to the one created later", user2, u1);
   u2 = (User) list.get(1);
   Assert.assertEquals("second one should be equal to the one created first", user1, u2);   
  }
  ...
  public void testGetUsersAfterDeletion() throws Exception {
   ...
   /* check the users now */
   User u1 = (User) list.get(0);
   Assert.assertEquals("first one's status should be set to deleted", (int)Status.Deleted.ordinal(), (int)u1.getStatus());
   User u2 = (User) list.get(1);
   Assert.assertEquals("second one should be equal to the one created later", user2, u2);
  }
